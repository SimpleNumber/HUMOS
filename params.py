...
#ms2length = (15, 15000) #length of a single MS/MS scan in milliseconds (DOI: 10.1021/pr500985w)
LC_time = 60 #LC method length in minutes
resolutions_list = [7500*(2**x) for x in range(0,6)] #list of resolutions
#resolutions_list = [7500*(2**x) for x in range(0,6)] # list of resolutions for ms2
agc_list = [1e5, 3e5, 5e5, 1e6, 3e6, 1e7] #list of AGC targets
transients = dict([(resolutions_list[x], 16*(2**x) + 9) for x in range(0,6)]) # transients and overhead DOI: 10.1021/pr500985w
...
IT_MS2_descript = 'The maximum injection time for MS2 spectra is the time allowed to be spent to accumulate ions and potentially reach the AGC target value.'
